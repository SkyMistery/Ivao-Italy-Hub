...
- **A4a** (nota `decisions/2026-09-26-le-parole-di-piu-moduli.md`, caso (b), decisa da Carmine sulla #133): con il training l'hub
  non partiva, perché `_source` e `nav.section` stanno in ogni file di modulo. Ora il catalogo del server tiene le chiavi di un
  modulo anche sotto il suo namespace, senza namespace solo quelle che un solo modulo dichiara, e rifiuta ancora i doppioni che
  toccano il nucleo.
- **Le chiavi dei moduli con il namespace** (nota `decisions/2026-09-26-le-chiavi-dei-moduli-con-il-namespace.md`, caso (b),
  decisa da Carmine in chat su proposta del revisore): una lettura senza namespace smetteva di rispondere, in silenzio, appena un
  altro modulo dichiarava la stessa chiave. Le 12 chiavi che il C# dei tour leggeva senza namespace passano a `flightops:…`, un test di architettura lo tiene per
  ogni modulo, e la riga di `CONTRIBUTING.md` si rovescia (#138).
- Il seme dei contenuti lascia com'è un template, una pagina o una dashboard già scritti a quell'indirizzo, come i tipi del
  calendario di A2 (#137, senza nota: applica la regola di A2 al resto dello stesso seeder).

**Changelog 1.14** (26 set 2026, dopo il merge di #129 e #131): **gli esaminatori, e A2 e A3 nel piano**.

- **Gli esaminatori.** Nota `decisions/2026-09-26-gli-esaminatori.md`, caso (a). **Una risposta di Carmine**: gli esami si
  assegnano solo a HQ, TC, TAC e TA, mai ai trainer. Corregge la decisione n.10 del design di M3: `Training.ManageExams` va a TC,
  TAC e TA1–9, non ai trainer. Nella stessa PR sono allineati `07-design-m3.md` (§2.8, §3.2, §3.4, §10, §12 n.10) e
  `08-piano-implementazione-m3.md` (A3b, A4, A10).
- **Cambiare e togliere un esame** spetta a HQ, TC, TAC e al TA a cui è assegnato: risposta 4 di Carmine sulla #131, fase del
  nucleo **A3b** prima di A10 (§9.2, §13).
- **A2** (nota `2026-09-25-le-postazioni-atc-e-il-tipo-exam`):
  - le postazioni ATC del mondo in `ref_ivao_atc_positions`, e quelle della divisione come domanda del nucleo (§7, §9.1);
  - gli endpoint veri, con `mapType` (§10);
  - `exam` nel seme dei tipi del calendario, con il seme che lascia com'è un tipo scritto a mano (§9.5).
- **A3** (nota `2026-09-25-i-permessi-alternativi-e-la-creazione`): `[AlsoWrittenWith]` ripetibile, e uno segnato
  `AlsoOnCreation` vale anche alla creazione (§16 punto 2).
- **Carmine** ha anche deciso di tenere il membro di default di `IIvaoApiClient.GetAtcPositionsAsync` (commento sulla #129).

**Changelog 1.13** (25 set 2026, dopo il merge di #121, #125 e #128): **M3 nel piano**, portato dal revisore dalle sezioni «Da
portare nel piano» del design `07-design-m3.md` (§14) e delle note delle fasi A0 e A1, tutte decise da Carmine sulla PR #121 e sulla
#125. **Il teorico lo dichiara il trainee**, dietro `ITheoryExamSource`, e chi approva lo controlla su IVAO: è uno scostamento dal
piano, al posto degli esiti inseriti dallo staff (nota `il-teorico-lo-dichiara-il-trainee`; §9.2, §14). **PATS resta acceso solo per
il feed del calendario dei trainer** fino all'iCal del nucleo in M6; §15.9 resta aperta (nota `che-cosa-resta-fuori-da-m3`; §9.2,
§13). **Group training fuori da M3**; lo storico di PATS come archivio in sola lettura, se se ne capiscono i codici (§9.2, §15.7).
**Il trainer conduce con un grant con scope** (nota `chi-conduce-e-chi-scrive-un-training`; §9.2). **Niente VID ai visitatori** nel
calendario pubblico (nota `il-training-in-pubblico`; §9.7). **Le pagine del membro sotto `/training`**, e `/me` riceve il blocco
`training.myTraining` (§8.2). **Un'eccezione dichiarata all'unico handler**: la risposta dello staff toglie le note riservate al
trainee della riga (nota `le-note-riservate-e-il-trainee`; §16 punto 2). **A1, nucleo** (nota `le-ore-e-il-vocabolario-dei-rating`):
le ore di connessione in `hub_users` (§7, §10, §6.4, §14 GDPR) e il vocabolario dei rating nel perimetro IVAO (§4.2, §9.1);
`RatingBadge` nell'elenco chiuso (§8.3). Le due estensioni dell'handler (`[AlsoWrittenWith]` ripetibile, A3; i capi FIR, A11)
entrano in §16 quando le note delle loro fasi sono decise. Nella stessa PR: `IvaoUserProfileReaderTests.RealShape` ha la forma vera
di `hours` (un array di righe in secondi, misurata in A1, non l'oggetto inventato prima) e ne controlla le ore; il commento di
`IvaoUserProfileReader` cita §6.4, dove sta la regola GDPR, non un §11.4 che non esiste più.

**Changelog 1.12** (25 set 2026, T20c): **M2 è chiusa.** Nota `decisions/2026-09-25-le-rifiniture-di-m2.md`: Carmine ha preso le
quattro raccomandazioni. (1) **Un modulo porta i suoi componenti dal manifest** (`ModuleManifest.components`, caso b): `LegGrid` è in
galleria come `flightops.LegGrid`, con un campione del modulo fatto di dati d'esempio (§8.3). (2) La **chiusura di un tour** è una voce
di calendario `deadline`, il rilascio resta `tour` (§9.5). (3) **I PIREP restano fuori** da ricerca e calendario, e un test lo fissa.
(4) Il rapporto si conta **dal merge della #70**. Il **giro completo** dei tour è una spec sola, `tours-round.spec.ts`: un tour da
template, due leg da un XLSX, il volo sul tracker finto, il controllo che segnala, rifiutato, contestato, accolto, accettato; la seconda
leg rifiutata per sbaglio, **riaperta** e accettata; il tour completato e l'award assegnato. La checklist del fork XX controlla anche le
impostazioni dei tour, la loro pagina pubblica e i due tipi di calendario. `FORKING.md` ha una sezione sui tour; `UI-GUIDELINES.md` §3
dice come un modulo porta un componente. Trovato guardando la galleria: l'editor delle leg diceva in inglese «2 legs flown» di leg che
nessuno aveva volato (l'italiano diceva «da volare»). **Il rapporto** è `decisions/2026-09-25-m2-review.md`: 21 tabelle `fo_` (19
...
`decisions/2026-09-25-le-fasi-in-coda.md`, caso (c). **Due risposte di Carmine**: (1) il revisore pubblica da solo i rilievi
sulle PR del collaboratore e ne controlla le correzioni, senza mai rispondere a una domanda che è di Carmine; (2) il
collaboratore **non aspetta il merge** per la fase dopo: il branch parte da quello della fase prima, la PR va **sempre a
`main`**, in bozza con `(after #N)` finché la fase sotto non è unita; Carmine unisce in ordine. Toccati: `CLAUDE.md` §0 (la
riga del revisore, la regola 4), `CONTRIBUTING.md` («Phases in a queue», al posto del divieto di impilare).
...
- **«Questo nomina IVAO?»** — la stessa domanda della riga sopra, un livello più in alto. Il codice
  ...
  `StaffRoleMap`), le tabelle `ref_ivao_*` e l'enum `Department` di `Division/Vocabulary.cs`. **Dal 25 set 2026**
  (M3, A1) ne fa parte anche il **vocabolario dei rating** (`Core/Ivao/RatingVocabulary.cs`): l'ordine, le sigle, quali
  rating hanno un training pratico e su che tipo di postazione. Un modulo gli fa domande e non scrive numeri di rating
  (nota `2026-09-25-rating-e-postazioni-dal-nucleo`).
...
|---|---|---|---|
| `users` | `vid` | `first_name`, `last_name`, `division_code`, `country`, `rating_atc`, `rating_pilot`, `hours_atc`, `hours_pilot`, `discord_id`, `locale`, `is_staff`, `is_superadmin`, `last_login_at`, `created_at` | fonte: `/v2/users/me` ad ogni login (le ore dal 25 set 2026, M3 A1: in ore, dai secondi di IVAO; restano le ultime quando il profilo non le porta); `is_superadmin` solo da bootstrap o da un altro superadmin |
...
| `ivao_atc_positions` | `callsign` | `position_type`, `airport_icao` o `center_id` (senza FK), `name`, `raw_json`, `synced_at` | **dal 25 set 2026** (M3, A2, nota `2026-09-25-le-postazioni-atc-e-il-tipo-exam`): snapshot **del mondo** di `/v2/ATCPositions/all` e `/v2/subcenters/all`, potato metà per metà e mai su una risposta vuota; indici su tipo, aeroporto e FIR. Quali sono della divisione lo dice `IAtcPositionDirectory` |
...
/me                        Dashboard personale; /me/profile, /me/bookings, /me/tours; il training entra con il blocco
                           training.myTraining, e le pagine del membro stanno sotto /training (/training/request,
                           /training/mine: design M3 §0.4, nota 2026-09-25-il-training-in-pubblico)
...
Componenti custom (pochi, costruiti con i token): `Hero` (gradiente atmos-800→atmos-600, eyebrow verde, CTA), `StatTile` (numero grande + etichetta, dati vivi), `SectionHeader` (eyebrow + titolo, come nel template HQ), `LiveStatusStrip` (ATC/piloti online), `RatingBadge` (nell'elenco chiuso dal 25 set 2026, il ventiquattresimo: un badge di testo, M3 A1), `AirportCard`, `EventTimeline`, `LocaleSwitcher`, `MarkdownContent`, `ContactForm` (visibile solo autenticati). Footer con link legali HQ (Terms of Use, Privacy Policy, IP Policy) e "Staff area".
...
`catalog.ts`, perché la galleria non importa da `modules/`. **Da T20c** (25 set 2026, piano 1.12, nota
`2026-09-25-le-rifiniture-di-m2`) un modulo porta i suoi componenti **dal manifest** (`components: [{ name, sample }]`) e la galleria li
mostra in «Componenti dei moduli» come `flightops.LegGrid`; il test della galleria ne scrive i nomi, perché aggiungerne uno resta una
decisione come aggiungerne uno qui. `ConfirmDialog` è
...
| Dati di riferimento IVAO | FIR/centri, aeroporti (snapshot giornalieri) | API IVAO `client_credentials` | §7 schema `ref_`; **dal 16 set 2026** (M2, T1) aeroporti **del mondo** con IATA e coordinate, piste con le testate (per gli aeroporti che le chiedono), tipi di aereo con varianti, equipaggiamenti e transponder; **dal 25 set 2026** (M3, A1) il vocabolario dei rating di IVAO, dati nel codice perché nessun endpoint lo elenca (`Core/Ivao/RatingVocabulary.cs`); **dal 25 set 2026** (M3, A2) le postazioni ATC del mondo, e le postazioni della divisione su cui si allena un rating come domanda del nucleo (`IAtcPositionDirectory`) |
...
- Tabella `cms_calendar_entries` nel nucleo (§7). I moduli **non** scrivono direttamente: le loro entità (evento, sessione di training, leg di tour) implementano `IProjectable` e l'interceptor EF del nucleo crea/aggiorna/rimuove la voce con `source_module` + `source_id` **nella stessa transazione** (§16.4). Le voci interne (`meeting`, `deadline`, `other`) le crea lo staff a mano nel proprio spazio di dipartimento. **Dal 25 set 2026** (M3, A2) `exam` è nel seme dei tipi; il seme lascia com'è un tipo che il back office ha già scritto con la stessa chiave, invece di far fallire l'avvio. **Dal 25 set 2026** (T20c, piano 1.12, nota `2026-09-25-le-rifiniture-di-m2`) un tour proietta il **rilascio** come `tour` e la **chiusura** come `deadline`, perché le due voci non si leggano uguali: un blocco che mostra solo `tour` mostra i rilasci. Un PIREP non proietta né ricerca né calendario.
...
ha fatto il login, sulla pagina della sessione.
...
|---|---|---|---|
| Profilo al login | `/users/me` | token utente | ad ogni login; dal 25 set 2026 anche le ore di connessione (`hours`: righe `{ type, hours }` in secondi; l'hub legge `atc` e `pilot`, M3 A1) |
...
| Posizioni ATC | `/v2/ATCPositions/all` e `/v2/subcenters/all`, sempre con `mapType=regionMapPolygon` (senza, i settori superano i 15 secondi del gateway di IVAO; misurato il 25 set 2026) | client_credentials | giornaliero, con il resto dei dati di riferimento (M3, A2) |
...
| Scope `training` non implementato lato IVAO → esiti teorici non leggibili via API | **Dal 25 set 2026** (M3): il trainee dichiara di aver superato il teorico alla richiesta, e chi approva lo controlla su IVAO; tutto dietro `ITheoryExamSource`, nel modulo, da sostituire quando l'API esporrà gli esiti (nota `2026-09-25-il-teorico-lo-dichiara-il-trainee`). L'inserimento manuale/CSV dallo staff è scartato: lo staff gli esiti non li ha. |
...
| GDPR (dati personali di membri, email, discord id, ore di connessione) | Minimizzazione scope, retention, export/cancellazione, privacy policy divisionale. Ogni dato IVAO in più ha uno scopo scritto: l'email per le notifiche (6 set 2026), le ore per le soglie del training (25 set 2026); la cancellazione dei dati di una persona li toglie (§16 punto 16). |
...
7. **Accesso ai DB esistenti** (PATS, sito Blazor) per stimare le migrazioni — `ivao-booking` non serve più (nessun import). Chi mantiene PATS oggi? **Il dump di PATS c'è** (12 set 2026, fornito dal TD e tenuto fuori dal repository perché ha dati personali): si importa senza errori, ma il significato dei codici numerici sta nel codice PHP di PATS, che non abbiamo. Resta da ottenerlo, per l'archivio in sola lettura di M3 (fase A12c).
