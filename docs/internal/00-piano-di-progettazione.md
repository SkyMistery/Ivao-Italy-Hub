...
**Versione documento:** 0.19 — 2 settembre 2026 (design di M0 + confine dei moduli anche nel frontend)
...
| La cartella dell'app **è stata il document root**: `appsettings.Production.json` fu scaricabile (24–25 ago); ora davanti c'è **Cloudflare** e le direttive nginx negano i file sensibili | I segreti stanno in `segreti/<nome-non-indovinabile>.json` (l'app carica ogni `*.json` di quella cartella, che vince su appsettings); deny nginx su `appsettings*.json`, `*.dll`, `*.pdb`, `diagnostica/`, `segreti/`, `keys/`. Forwarded headers da Cloudflare. |
...
│   └── IvaoHub.Modules.SpecialOps/ # SO dept (OPZIONALE, modules.specialops): contenuto da definire col dipartimento
...
2. **Cartella dell'app** nella sottoscrizione `it.ivao.aero`, avviata da **Passenger** (`dotnet IvaoHub.Web.dll`), `ASPNETCORE_ENVIRONMENT=Production`. Struttura: `wwwroot/`, `config/division.json`, `config/ivao-oauth.json` (compilato dalla divisione), `segreti/<nome-non-indovinabile>.json` (connection string, SMTP, secret — l'app carica ogni `*.json` di `segreti/`), `hub-keys/` (Data Protection, **persistente, mai cancellare**), `uploads/` (documenti), `logs/`, `diagnostica/`.
3. **Direttive nginx aggiuntive** in Plesk: `deny all` su `segreti/`, `hub-keys/`, `diagnostica/`, `logs/`, `appsettings*.json`, `*.dll`, `*.pdb`, `*.json` alla radice; `Cache-Control: no-store` su `/api/*` (Cloudflare davanti). Verifica dall'esterno con `curl -I` dopo ogni cambio di hosting.
...
5. **Migrazioni**: `Database.Migrate()` all'avvio (senza shell non c'è alternativa), con tre regole ferree: solo migrazioni **additive** (mai `DROP`/rename distruttivi nello stesso pacchetto che smette di usare la colonna → pattern *expand/contract* in due release), test CI che applica l'intera catena su una **MariaDB 11.4.10 vera**, e un `avvio-diagnostica.txt` che dice quale migrazione ha applicato. Niente consegne con migrazioni nelle finestre in cui nessuno può ripristinare.
6. **Aggiornamento**: upload via FTP in **binario**, rimettere il bit di esecuzione all'eseguibile, non toccare `hub-keys/`, `segreti/`, `uploads/`; poi `tmp/restart.txt`. Sonda post-deploy (`/api/version`, `/health`, login, una pagina per modulo) eseguita **non** nel minuto del riavvio.
...
| Segreti/chiavi esposti o persi via FTP (è già successo a vIPI il 24–25 ago) | Cartella `segreti/` con nome non indovinabile, deny nginx verificato con `curl -I`, `hub-keys/` nel foglio "non cancellare", rotazione credenziali a ogni sospetto. |
