...
> 25 set** (nota §9, id 5617). **M3 è partito**: il design è deciso (#121), A0 (#125), A1 (#128, il primo codice del nucleo), A2 (#129)
> e A3 (#131) sono unite il 25 set, e il piano le riporta dalla 1.13 e dalla 1.14. Gli esaminatori sono HQ, TC, TAC e i TA, mai i
...
> rifiutato** resta con lo pseudonimo, svuotato da `TourRetentionJob.Empty` (ora `internal static`, condiviso) e in più senza callsign,
...
**Precedente:** T0–T20a e la PR del nucleo di T20b, più la piccola estensione `ErasureRequest.Keep` (piano **1.09**, nota `2026-09-25-le-righe-che-restano-con-il-vid`: un ban in vigore la usa per restare con il VID, perché il nucleo scrive lo pseudonimo dopo l'eraser del modulo). **Il prossimo passo è la PR del modulo di T20b** (`FlightOpsPersonalData : IPersonalDataEraser`, design §10.0), dopo il merge di quella del nucleo; poi T20c (rifiniture, giro completo, chiusura di M2). T21 (l'app Python del validatore) sta fuori da questo repository e può andare in parallelo.
...
> `SchemaForm` che legge un numero vuoto come «nessun numero». Il briefing lo scrive T6b (sopra).
...
> è di **trenta giorni**, METAR **e** TAF; `tools/record-ivao-fixtures.mjs` registra altri voli quando arriva il corpus di Carmine.
>
