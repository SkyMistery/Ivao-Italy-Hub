...
5. **Sessioni scadute** (F8): ogni entità `IAffectsUserSession` con `AffectedVid > 0` toccata dalla scrittura fa rigenerare il `security_stamp` di quel VID **nella stessa transazione**, e la voce della cache viene tolta **dopo il commit** (prima sarebbe un invito a rileggere la riga vecchia). `UserGrant` è la prima e in M0 l'unica: cambiare un grant cambia che cosa il suo titolare può fare, quindi il cookie che sta portando smette di valere alla richiesta successiva invece che alle dodici ore. Un contesto di modulo che non ha `hub_users` nel proprio modello non fa niente, come già per le proiezioni. Nota `2026-09-04-grant-e-sessione.md`.

Registrato una volta in `AddHubDbContext`; ogni `DbContext` di modulo lo riceve dallo stesso metodo (`AddModuleDbContext<T>`), quindi non può essere «dimenticato». Un test di architettura vieta `AddDbContext<` fuori da quei due metodi: un contesto registrato a mano scriverebbe senza audit, senza guardia e senza proiezioni, e `MapCrud` lo servirebbe volentieri.
...
- **Nome**: `<Area>.<Azione>`; contributo del nucleo in `CorePermissions`: `Content.View`, `Content.Edit`, `Content.Publish`, `Content.ManageTemplates`, `Links.View`, `Links.Edit`, `Calendar.View`, `Calendar.Edit`, `Permissions.Manage`, `Modules.Manage`, `Audit.View`, `Awards.Assign`, `Admin.Access`. Regola del catalogo: ogni area dipartimentale dichiara **sempre** `View` ed `Edit`, e `Edit` implica `View` nel calcolo dei permessi effettivi (una riga in `EffectivePermissionsCalculator`, non nel handler). I moduli aggiungono i propri via `IModule.Permissions`.
- **Il catalogo è composto, non statico** (F8): `PermissionCatalog` è un singleton costruito da `AddHubModules` con i permessi del nucleo più quelli dei moduli **abilitati**, e lo interrogano il policy provider, `EffectivePermissionsCalculator` e il validatore di un grant — così un permesso di modulo è un permesso ovunque o in nessun posto. Due dichiarazioni dello stesso nome sono un'eccezione all'avvio. `PermissionCatalog.Core` è il catalogo di un hub senza moduli, ed è quello che usa chi non ha un container a cui chiedere (i test unitari, uno strumento di design time).
...
- **Grant** (`hub_user_grants`): `kind ∈ {permission}`, `value` = nome permesso, `department` nullable (null = tutti), `effect ∈ {grant, deny}`, `expires_at`, `suspended_at`, `granted_by`, `reason`; audit standard (`IAuditable`), **non** `IOwnedByDepartment` (è gestita in modalità globale da `MapCrud`, §3.9). Effettivi = derivati ∪ grant − deny. Vincoli server: grant solo a VID con `is_staff = true` (o superadmin); mai un permesso globale; e mai un nome fuori dal catalogo — le tre regole stanno in `GrantWriteDtoValidator` e rispondono con `errors.grant.notStaff`, `errors.grant.globalPermission`, `errors.grant.unknownPermission`. `UserGrant` è anche `IAffectsUserSession` (§3.4): scriverlo rinfresca lo stamp del suo titolare.
- **L'unica policy che non è un permesso**: `HubPolicies.SignedIn`, registrata esplicitamente in `AddIvaoAuthentication`. Serve a un endpoint che chiede solo di essere autenticati e niente altro — `PUT /api/me/locale` è il primo — e dichiara `AddAuthenticationSchemes(CookieScheme)` come tutte le altre. Non è una scappatoia: «essere autenticati» non è un permesso del catalogo, è il pavimento sotto tutti quanti, e c'è posto per esattamente una policy di questo tipo. Aggiunta in F6.
...
type Bootstrap = {
  user: null | { vid: number; firstName: string; lastName: string; locale: string; isStaff: boolean; isSuperadmin: boolean;
                 hasAllDepartments: boolean;   // il claim `alldept`, non una deduzione dalla lista dei permessi (§3.3)
  ...
  modules: { key: string; department: Department | null; enabled: boolean; maintenance: boolean }[];  // anche gli opzionali spenti, con enabled: false
  ...
  registries: { blocks: BlockDescriptor[]; widgets: WidgetDescriptor[];
                permissions: { name: string; isGlobal: boolean }[] };   // il catalogo, non i permessi di chi chiede
  version: string;
};
```
...
`modules` elenca **tutti** i moduli della build, non solo quelli accesi: dirlo è ciò che permette alla schermata di amministrazione di mostrare un modulo spento come qualcosa che si può riaccendere, invece che come qualcosa che non esiste. `registries.permissions` è il **catalogo** (§3.7) e non i permessi del chiamante: non è un'informazione sensibile — sta nel sorgente di ogni fork — e serve alla schermata dei grant, che senza di esso avrebbe una casella di testo dove va un nome che l'installazione conosce. Non c'è nessun `GET /api/admin/modules`: la stessa domanda con due risposte sarebbe la seconda cosa da tenere allineata.

`hasAllDepartments` è stato aggiunto in F6 perché la sidebar staff deve elencare **tutti** i dipartimenti a un director, al web team e a un superadmin. Il client non può dedurlo dalla forma della lista dei permessi per la stessa ragione per cui non lo fa il server (`docs/internal/decisions/2026-09-03-reaches-every-department.md`): un deny che espande un permesso in dipartimenti espliciti cambierebbe la risposta.

**`PUT /api/me/locale`** (`Core/Auth/LocaleEndpoints.cs`, F6): il membro sceglie la lingua, che viene validata con `LocalePreference.Spoken` — la stessa regola del login — scritta in `hub_users.locale` e **rimessa nel cookie** con `HubClaims.WithLocale`, perché `ICurrentUser.Locale` si legge da lì e altrimenti i `ProblemDetails` continuerebbero ad arrivare nella lingua precedente fino al login successivo. Una lingua che la divisione non parla prende 400 con la chiave `errors.locale.unsupported` sul campo `locale`. Il cookie `hub.lang` resta di competenza del browser: lo scrive `LocaleSwitcher`, ed è tutto ciò che ha un visitatore anonimo.
...
`POST /api/content/{id}/publish` (`Content.Publish` sul dipartimento): (1) `Title.HasAll(locales)` e walker «ogni `Localized` dentro `props` ha tutte le lingue» → altrimenti 400 con elenco dei percorsi mancanti; (2) per ogni blocco `kind = data` con `renderMode = frozen` chiama `IDataBlockProvider(type).ResolveAsync(props, context)` e scrive il risultato in `frozen`; (3) crea `ContentVersion` (`Version = max+1`), aggiorna `PublishedVersionId`, `Status = Published`, `PublishedAt`; (4) l'interceptor proietta nella ricerca. `GET /api/content/public/{kind}/{slug}` restituisce **solo** la versione pubblicata (filtro visibilità dal query filter); i blocchi Data `live` vengono risolti dalla SPA con `GET /api/blocks/data/{type}?props=<base64 json>` (stesso provider, stesso `ICurrentUser`). Un `IDataBlockProvider` **non** è un mini-modulo: è un servizio del nucleo o di un modulo, registrato per `type`.

**Il `context` (precisato in F7).** Un provider riceve un `DataBlockContext`, che dice **dove finirà
la risposta**: `null` sul percorso `live` — lì il lettore è il lettore, e il query filter di §3.5 ha
già detto l'ultima parola — e la `Visibility` più l'`OwnerDepartment` del contenuto quando la
risposta sta per essere **congelata**. Serve perché una cattura smette di essere letta da chi l'ha
chiesta e comincia a essere letta da chiunque apra la pagina: senza tetto, un coordinatore che
pubblica una pagina `Public` ci congelerebbe dentro le righe `Staff` che solo lui vede. Il tetto è
una tabella in `Core/Division/VisibilityCeiling.cs` (`Public` → solo `Public`; `Members` → `Public`
e `Members`; `Staff` → le tre aperte; `Department` → tutte, ma le righe `Department` solo del
**suo** dipartimento), non un ordinamento: `Department` è più stretta di `Staff` ma nomina persone
diverse per ogni dipartimento. Non è una seconda copia del query filter, che risponde a un'altra
domanda: «questo lettore può vedere questa riga» contro «questa riga può essere copiata dentro una
pagina che leggerà qualcun altro». Nota:
`docs/internal/decisions/2026-09-04-frozen-e-visibilita.md`.
...
`ModuleBase` implementa tutto come vuoto tranne `Key`: un modulo che porta un endpoint e una voce di menu scrive due membri, non undici.

`ModuleRegistry` (nucleo): riceve un **elenco esplicito** di moduli — `IvaoHub.Web/Modules.cs` contiene `public static readonly IModule[] All = [new AtcModule(), …]`, l'unico posto da toccare per aggiungere un modulo al backend — niente scansione «di tutto ciò che Web referenzia» (§6.5); esclude gli opzionali spenti in `division.modules`, espone `Enabled`, `IsInMaintenance(key)` (da `hub_division_settings['modules.<key>.maintenance']`, cache 5 s) e compone i contributi. Middleware `ModuleMaintenanceMiddleware`: `GET` sotto `/api/<key>` passano (sola lettura), altri verbi → 503 `ProblemDetails` con il titolo risolto da `errors.maintenance.title` e l'estensione `module`; i job del modulo controllano `IsInMaintenanceAsync` a inizio esecuzione. Sta **prima del routing**, quindi mentre un modulo è chiuso non accetta modifiche nemmeno un indirizzo che quel modulo non ha: «chiuso» vale per il prefisso, non per l'elenco delle rotte. Lo si accende con `PUT /api/admin/modules/{key}/maintenance` dietro `Modules.Manage`; la riga di audit la scrive l'interceptor, perché `DivisionSetting` è `[Audited]`.
...
- **`MarkdownContent`** rende Markdown con **`react-markdown`** (deciso da Carmine il 3 set 2026, `docs/internal/decisions/2026-09-03-markdown-content.md`): costruisce un albero React e non tocca mai `innerHTML`, e l'HTML grezzo nel sorgente **non** e' abilitato. Va in un chunk suo, caricato solo dalle pagine che mostrano prosa.
- **`/staff/admin/ui-kit`**: una route che monta ogni componente dell'elenco e ogni blocco del registry con props di esempio; test Vitest che verifica che ogni entry del registry compaia nella ui-kit. L'elenco chiuso vive in un file solo (`web/src/shared/ui/catalog.ts`) che leggono sia la ui-kit sia il test: una copia sarebbe il posto da cui un componente sparisce in silenzio.
...
- `DataList<TRow>({ columns, query, search, onSearchChange, labels, locale, defaultLocale, timezone, actions, toolbar, emptyAction })`: usa `DataTable` Atmosphere (TanStack Table) in modalità server-side; la paginazione/ordinamento/`q` sono **i search params della route** (ricetta 2). Colonne dichiarate in `features/<x>/list.ts` con helper `col.localized('title')`, `col.badge('visibility')`, `col.date('updatedAt')`, `col.department()` — descrizioni e non componenti, così quel file resta TypeScript senza JSX.
  **Precisato in F6**: prende `search` e `onSearchChange` invece dell'oggetto `route` che questo design abbozzava. Un componente generico che entrasse nel router dovrebbe riallargare i search params a `unknown`, cioè buttare via esattamente la tipizzazione per cui la ricetta 2 esiste; le due righe di collegamento stanno nel file di route, dove i tipi ci sono, e non aggiungono una riga di JSX di tabella. Documentato in `web/src/routes/README.md`.
  La paginazione la disegna `DataList` e non `DataTable`: quella di Atmosphere scrive «Rows per page» e «Page 1 of 3» in inglese, e nessuna schermata dell'hub porta una frase non tradotta. Stesso motivo per cui il menu di visibilità delle colonne è spento.
- `SchemaForm({ schema: zod, defaults, locales, labels: nsKey, onSubmit, submitLabel, secondaryAction })`: walk dello schema zod 4 → campi: `z.string()` → input/textarea (`.meta({ multiline: true })`), `z.number()`, `z.boolean()` → switch, `z.enum` → select, `localized()` (helper che produce un `z.record` con `.meta({ localized: true })`) → `LocaleFields` (tab per lingua + «copia dall'altra lingua»), `z.array` di oggetti → lista ripetibile, `z.object` annidato → fieldset. Un tipo che il generatore non conosce **lancia**, non viene saltato: un campo che sparisce in silenzio è un campo che nessuno può compilare. Terza annotazione, aggiunta in F6: **`.meta({ hidden: true })`**, per un campo che viaggia nel payload e non si disegna — `rowVersion` è la ragione per cui esiste. Quarta, aggiunta in F7: **`.meta({ choices: [1,2,3,4] })`** su un numero, che diventa una select — e non un `z.enum`, perché ogni stringa dentro le `props` di un blocco viene estratta come testo della pagina per l'indice di ricerca, e il livello di un titolo non è testo (§5.3). Quinta, aggiunta in F8: **`.meta({ choices })` vale anche per una stringa**, e le opzioni sono i valori stessi — nessuna chiave i18n per ciascuno. Serve a un insieme che si conosce solo a runtime, e il catalogo dei permessi è il primo: che cosa contenga dipende da quali moduli sono installati, quindi non può essere un `z.enum`, e i suoi membri sono identificatori e non prosa (`Links.Edit` si scrive `Links.Edit` in ogni lingua, come un VID o un codice di dipartimento). Sempre in F7: una **`z.enum` opzionale** disegna anche la voce «nessuno» (etichetta `<ns>.options.<path>.none`), perché una select non ha un gesto per tornare indietro e senza quella voce la prima scelta sarebbe definitiva; e il generatore legge il **`.default()`** che un campo dichiara, così «con cosa nasce un blocco nuovo» sta accanto al campo e non in un secondo posto. Etichette da i18n `<ns>.fields.<path>` e scelte di una select da `<ns>.options.<path>.<value>`; dentro una lista ripetibile l'etichetta usa il path dello schema (`aliases.name`) e l'indice resta solo nel nome del campo del form. Lo stesso generatore serve al **form proprietà dei blocchi** e alle entità (`links`, `content` metadati, `grants`). Per le entità lo schema zod è scritto a mano in `features/<x>/schema.ts` **specchio del DTO** (test Vitest che confronta le chiavi con `schema.d.ts`).
- **`<ns>.hints.<campo>`** (aggiunto il 5 set 2026): il generatore disegna una frase sotto un campo quando i file di lingua ne portano una, e niente altrimenti — **nessun flag nello schema**, perché un suggerimento è parole e le parole stanno in `locales/`. Esiste perché un'etichetta faceva due lavori: `<ns>.fields.<campo>` è insieme l'etichetta del form e l'**intestazione di colonna** di `DataList`, e un'etichetta che portava il formato («Expires (YYYY-MM-DD, empty for never)») dava alla tabella dei grant un'intestazione alta cinque righe. Separando i due, l'intestazione si accorcia da sé e non nasce una seconda chiave da tenere allineata. Attenzione: i18next restituisce **la chiave** quando manca, quindi la lettura passa da `i18n.exists()` — senza, metà form mostrerebbe `<ns>.hints.<campo>`.
- **`LocaleFields` passa `className="w-full"` a `Tabs`**: quel componente di Atmosphere si pinna a `w-[400px]`, quindi un campo tradotto era largo 400 px accanto a input larghi quanto il form. La classe si fonde perché passa da `cn`.
...
Route `/staff/$dept/content/$id`: pannello sinistro albero sezioni/blocchi (aggiungi sezione, aggiungi blocco tra gli `allowedBlocks`, sposta su/giù, duplica, elimina; sezioni `locked` mostrano solo «modifica props»), pannello destro `SchemaForm` del blocco/sezione selezionata, in alto metadati (`SchemaForm` dei metadati con `LocaleFields`), pulsanti Salva bozza / Pubblica / Anteprima (renderer con la bozza). **Precisato in F7**: una bozza non porta nessuna cattura — la pubblicazione la scrive nella versione, non all'indietro nella bozza — quindi l'anteprima di un blocco `frozen` mostra per forza dati **live**, ed è giusto così; il badge dice quale delle due cose si sta guardando («catturato alla pubblicazione» su una versione, «ora dal vivo, catturato quando pubblichi» su una bozza), e lo vede solo lo staff. Differenze rispetto al template (sezioni nuove/tolte): **M1**.

---

## 8. Test della spina dorsale (obbligatori, rompono la build)

Unit (`IvaoHub.UnitTests`): `StaffRoleMapTests` (tabella completa, ordine dei pattern, XX/XXX, FIR), `LocalizedTests` (converter round-trip, `Resolve`, `HasAll`), `RolePermissionMatrixTests` (ogni riga), `EffectivePermissionsTests` (derivati ∪ grant − deny, scadenza, sospensione, vincoli), `BlockDocumentWalkerTests` (envelope, profondità, testo per lingua, ordine), `VisibilityCeilingTests` (la tabella riga per riga), `DivisionOptionsValidationTests`, `IvaoOAuthOptionsValidationTests`, `ArchitectureTests` (riferimenti tra progetti), `PolicyNamesTests` (ogni policy usata esiste nel catalogo).
...
Frontend (Vitest): schemi zod dei blocchi (props di esempio validi), `SchemaForm` per ogni tipo di campo, `LocaleFields`, `useProblemDetails`, registry ⇄ ui-kit, `i18n:check`, schema-entità ⇄ `schema.d.ts`, e **`Chrome.test.tsx`**, che monta il frame dentro `HubProviders` — l'albero dei provider vero, non una copia — perché un test che elenca i provider per conto suo resta verde con l'applicazione rotta.

**Playwright (`pnpm e2e`), bloccante in CI.** Tre smoke su Chromium contro il bundle di produzione servito da `vite preview`, con `/api/me` risposto da `e2e/fixtures.ts`: la home dentro il suo guscio, i controlli dell'header (tooltip compreso, aperto davvero con un hover) e un indirizzo inesistente che non deve essere un crash. Era «solo `pnpm e2e`, non bloccante in M0» fino al 4 set 2026, quando un `TooltipProvider` mancante ha ucciso ogni schermata dietro un layout con 74 Vitest verdi: uno smoke che non può fermare una release non è una rete. Nota `decisions/2026-09-04-smoke-in-un-browser.md`. Resta scoperto il giro con l'API vera e una `/{slug}` pubblicata da un seed, che il design immaginava: è un debito, HANDOFF §10.
...
16. Un grant invalida la sessione del suo titolare attraverso l'**entità** (`IAffectsUserSession`) e non attraverso un secondo gancio di `MapCrud`: vale per chiunque scriva la riga, non solo per chi passa dal motore. Nota `2026-09-04-grant-e-sessione.md` (4 set 2026).
17. Il catalogo dei permessi è **composto** (nucleo ∪ moduli abilitati) in `PermissionCatalog`, un singleton, invece di restare la lista statica di `CorePermissions`: un permesso di modulo deve diventare una policy come uno del nucleo (§3.7).
18. La manutenzione di un modulo si accende con `PUT /api/admin/modules/{key}/maintenance` e si **legge da `/api/me`**: non esiste un `GET /api/admin/modules`, perché il client ha già bisogno di quello stato per disegnarsi e una seconda risposta alla stessa domanda è una seconda cosa da tenere allineata (§3.10).
