...
/**
 * What sits behind a section. Four since G3, seven since 11 September 2026, when Carmine asked
 * for the palette of va.ivao.aero's page builder — `brand`, `deep` and `dark` are dark grounds,
 * drawn in the dark theme so that what is on them reads by construction (the renderer explains
 * how) — and **eight since 12 September**, when `aurora` joined them and `accent` stopped being a
 * grey. There is still no free colour, and deliberately — that is the one piece of theirs not
 * taken, because a colour chosen by hand cannot promise that the text on it can be read.
 *
 ...
 */
export const BACKGROUNDS = ['none', 'muted', 'accent', 'brand', 'deep', 'dark', 'aurora', 'image'] as const;
