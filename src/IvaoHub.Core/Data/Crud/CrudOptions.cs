    ...
    where TEntity : class
{
    ...
    /// <summary>
    /// The rows of one page of the list at once, in place of <see cref="ToList"/> row by row, for a
    /// list row that shows something its own columns do not hold. It is handed the page already
    /// read, so what it adds costs a query per page and never one per row.
    /// <para>The media library is the reason it exists (M2, T4): "this file will be deleted on…"
    /// is a fact of the rows of modules that use the file, and the library shows it next to the file
    /// (note 2026-09-15-file-con-scadenza §3). The engine still knows nothing of either.</para>
    /// </summary>
    public Func<IReadOnlyList<TEntity>, IServiceProvider, CancellationToken, Task<IReadOnlyList<TListDto>>>? ToListPage { get; set; }
